...
void ql_pricingengines::mceuropeangjrgarchengine(py::module_& m) {
    m.def("MCEuropeanGJRGARCHEngine",
        [](const ext::shared_ptr<GJRGARCHProcess>& process,
           ...
           BigNatural seed) -> ext::shared_ptr<PricingEngine> {
            ...
            if (lowerRngType == "pseudorandom") {
                auto maker = MakeMCEuropeanGJRGARCHEngine<PseudoRandom>(process);
                ...
                auto maker = MakeMCEuropeanGJRGARCHEngine<LowDiscrepancy>(process);
